...
function LlenarComboEmpleado() {
    LlenarComboXServicios("http://localhost:53634/api/Empleado", "#cboEmpleado");
}
async function LlenarComboTipoProducto() {
    await LlenarComboXServicios("http://localhost:53634/api/TipoProducto", "#cboTipoProducto");
    LlenarComboProducto();
}
async function LlenarComboProducto() {
    //Leer el Tipo de producto
    let TipoProducto = $("#cboTipoProducto").val();
    await LlenarComboXServicios("http://localhost:53634/api/Producto?CodigoTipoProducto=" + TipoProducto, "#cboProducto");
    //Capturar el código de producto y se va presentar en el input
    CalcularSubtotal();
}
...
async function ConsultarCliente() {
    ...
    try {
        ...
        const Rpta = await Respuesta.json();
        //Se presenta la respuesta en el div mensaje
        $("#txtNombreCliente").val(Rpta.Nombre + " " + Rpta.PrimerApellido + " " + Rpta.SegundoApellido);
    }
    catch (error) {
        //Se presenta la respuesta en el div mensaje
        $("#dvMensaje").html(error);
